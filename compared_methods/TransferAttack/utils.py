...
class ImageTextDataset(torch.utils.data.Dataset):
    def __init__(self, it_pair_path, image_path, transform=None):
...
class ImageFolderWithPaths(torchvision.datasets.ImageFolder):
    def __getitem__(self, index: int):
        image_path, _ = self.samples[index]  # path: str
        image = Image.open(image_path)
        image = image.resize((img_height, img_width)).convert('RGB')
        image = np.array(image).astype(np.float32) / 255
        image = torch.from_numpy(image).permute(2, 0, 1)
        # image = image.to(torch.float16)
        return image
class FixedCosineEmbeddingLoss(nn.Module):
    def __init__(self):
        super(FixedCosineEmbeddingLoss, self).__init__()
        self.loss_fn = nn.CosineEmbeddingLoss()

    def forward(self, input1, input2):
        # 假设 target 始终为 1，表示 input1 和 input2 应该是相似的
