...
class SASD_WS(MIFGSM):
    ...
    def __init__(self, model_name, epsilon=8 / 255, alpha=1.0 / 255, epoch=300, decay=1., p=0.93, targeted=False,
                 random_start=False,
                 norm='linfty', loss='crossentropy', device=None, attack='SASD_WS',
                 checkpoint_path='/new_data/yifei2/junhong/text_guide_attack/saved_result/saved_model',
                 feature_layer='layer4',**kwargs):
    ...
    def load_model(self, model_name, **kwargs):
        ...
        if not os.path.exists(weight_path):
            raise ValueError(
                "Please download the checkpoint of the 'resnet50_SASD_Model' from 'https://drive.google.com/drive/folders/1CsNN53GYy9nFcJdSkS5Pcy_faisMDRRh', and put it into the path '{}'.".format(
        ...
        sasd_ws_model = torch.nn.Sequential(transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
                                            sasd_ws_model)
    ...
    def forward(self, data, label, **kwargs):
        ...
        for _ in range(self.epoch):
            # Calculate the loss
            feature_clean = self.get_logits(self.DI(data + delta)).view(batch_size,-1)
            # feature_clean = mid_output.view(batch_size, -1)

            # feature_target = self.get_logits(self.DI(label)).view(batch_size,-1)
            feature_target = self.get_logits(label).view(batch_size, -1)
            # feature_target = mid_output.view(batch_size, -1)
