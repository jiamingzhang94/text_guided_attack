...
def compute_txt_sim(args, clip_model):
    ...
    with open(os.path.join(args.pred_text_path), 'r') as f:
        ...
        # pred_text=[i[:77] for i in pred_text]
        f.close()
    ...
    tgt_text_features = []
    with torch.no_grad():
        tgt_text_token = clip.tokenize(tgt_text, truncate=True).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pred_text_path", default='/mnt/sdc1/junhong/proj/AttackVLM-main/output/unidiffuser/caption_1000.txt', type=str)
    parser.add_argument("--tgt_text_path",  default='/mnt/sdc1/junhong/proj/AttackVLM-main/data/captions/coco_captions_1000.txt', type=str)
    ...
    if not args.clip_encoder:
        for model_type in clip_encoder_list:
            ...
            clip_model, _ = clip.load(model_type, device=device,download_root="model/clip")
            clip_model.eval()
