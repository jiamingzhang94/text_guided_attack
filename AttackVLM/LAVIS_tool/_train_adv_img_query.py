...
transform_a = torchvision.transforms.Compose(
    [
        ...
        torchvision.transforms.Resize(size=(384, 384), interpolation=torchvision.transforms.InterpolationMode.BICUBIC,
...
if __name__ == "__main__":
    ...
    # load models for i2t
    model, vis_processors, txt_processors = load_model_and_preprocess(name=args.model_name, model_type=args.model_type,
    ...
    clip_img_model_vitl14, _ = clip.load("ViT-L/14", device=device, jit=False,
                                         download_root="/new_data/yifei2/junhong/AttackVLM-main/model/clip")
    # clip_img_model_rn50 = torch.jit.load("/new_data/yifei2/junhong/AttackVLM-main/model/clip/RN50.pt", map_location=device)
    # clip_img_model_rn101 = torch.jit.load("/new_data/yifei2/junhong/AttackVLM-main/model/clip/RN101.pt", map_location=device)
    # clip_img_model_vitb16 = torch.jit.load("/new_data/yifei2/junhong/AttackVLM-main/model/clip/ViT-B-16.pt", map_location=device)
    # clip_img_model_vitb32 = torch.jit.load("/new_data/yifei2/junhong/AttackVLM-main/model/clip/ViT-B-32.pt", map_location=device)
    # clip_img_model_vitl14 = torch.jit.load("/new_data/yifei2/junhong/AttackVLM-main/model/clip/ViT-L-14.pt", map_location=device)
    # print("Done")
    # ---------------------- #
    ...
    if args.input_res == 384:
        ...
        clean_data = ImageFolderWithPaths("/new_data/yifei2/junhong/AttackVLM-main/data/imagenet-1K",
    ...
    with open(os.path.join(adv_vit_text_path), 'r') as f:
        ...
        # lavis_text_of_adv_vit = [i[:77] for i in lavis_text_of_adv_vit]
        f.close()

    with torch.no_grad():
        adv_vit_text_token = clip.tokenize(lavis_text_of_adv_vit,truncate=True).to(device)
    ...
    with open(os.path.join(tgt_text_path), 'r') as f:
        ...
        # tgt_text = [i[:77] for i in tgt_text]
        f.close()
    ...
    query_attack_results = torch.sum(adv_vit_text_features * target_text_features,
    ...
    with torch.no_grad():
        ...
        vit_attack_results_rn50 = torch.sum(adv_vit_text_features_rn50 * target_text_features_rn50,
                                            dim=1).squeeze().detach().cpu().numpy()
        query_attack_results_rn50 = torch.sum(adv_vit_text_features_rn50 * target_text_features_rn50,
                                              dim=1).squeeze().detach().cpu().numpy()
        ...
        adv_vit_text_features_rn101 = adv_vit_text_features_rn101 / adv_vit_text_features_rn101.norm(dim=1,
                                                                                                     keepdim=True)
        ...
        vit_attack_results_rn101 = torch.sum(adv_vit_text_features_rn101 * target_text_features_rn101,
                                             dim=1).squeeze().detach().cpu().numpy()
        query_attack_results_rn101 = torch.sum(adv_vit_text_features_rn101 * target_text_features_rn101,
                                               dim=1).squeeze().detach().cpu().numpy()
        ...
        adv_vit_text_features_vitb16 = adv_vit_text_features_vitb16 / adv_vit_text_features_vitb16.norm(dim=1,
                                                                                                        keepdim=True)
        ...
        target_text_features_vitb16 = target_text_features_vitb16 / target_text_features_vitb16.norm(dim=1,
                                                                                                     keepdim=True)
        target_text_features_vitb16 = target_text_features_vitb16.detach()
        vit_attack_results_vitb16 = torch.sum(adv_vit_text_features_vitb16 * target_text_features_vitb16,
                                              dim=1).squeeze().detach().cpu().numpy()
        query_attack_results_vitb16 = torch.sum(adv_vit_text_features_vitb16 * target_text_features_vitb16,
                                                dim=1).squeeze().detach().cpu().numpy()
        ...
        adv_vit_text_features_vitl14 = adv_vit_text_features_vitl14 / adv_vit_text_features_vitl14.norm(dim=1,
                                                                                                        keepdim=True)
        ...
        target_text_features_vitl14 = target_text_features_vitl14 / target_text_features_vitl14.norm(dim=1,
                                                                                                     keepdim=True)
        target_text_features_vitl14 = target_text_features_vitl14.detach()
        vit_attack_results_vitl14 = torch.sum(adv_vit_text_features_vitl14 * target_text_features_vitl14,
                                              dim=1).squeeze().detach().cpu().numpy()
        query_attack_results_vitl14 = torch.sum(adv_vit_text_features_vitl14 * target_text_features_vitl14,
                                                dim=1).squeeze().detach().cpu().numpy()
    ...
    for i, ((image, _, path), (image_clean, _, _)) in enumerate(zip(data_loader, clean_data_loader)):
        ...
        for step_idx in range(args.steps):
            ...
            if step_idx == 0:
                image_repeat = image.repeat(num_query, 1, 1,
                                            1)  # size = (num_query x batch_size, 3, args.input_res, args.input_res)
            else:
                image_repeat = adv_image_in_current_step.repeat(num_query, 1, 1, 1)

                lavis_text_of_adv_image_in_current_step = _i2t(args, txt_processors, model,
                                                               image=adv_image_in_current_step)
                # print("lavis_text_of_adv_image_in_current_step:")
                # print(lavis_text_of_adv_image_in_current_step)
                # lavis_text_of_adv_image_in_current_step = [i[:77] for i in lavis_text_of_adv_image_in_current_step]
                adv_vit_text_token_in_current_step = clip.tokenize(lavis_text_of_adv_image_in_current_step,truncate=True).to(device)
                adv_vit_text_features_in_current_step = clip_img_model_vitb32.encode_text(
                    adv_vit_text_token_in_current_step)
                adv_vit_text_features_in_current_step = adv_vit_text_features_in_current_step / adv_vit_text_features_in_current_step.norm(
                    dim=1, keepdim=True)
            ...
            perturbed_image_repeat = torch.clamp(image_repeat + (sigma * query_noise), 0.0,
                                                 255.0)  # size = (num_query x batch_size, 3, args.input_res, args.input_res)

            # num_query is obtained via serveral iterations
            text_of_perturbed_imgs = []
            for query_idx in range(num_query // num_sub_query):
                sub_perturbed_image_repeat = perturbed_image_repeat[
                                             num_sub_query * (query_idx): num_sub_query * (query_idx + 1)]
                if args.model_name == 'img2prompt_vqa':
            ...
            with torch.no_grad():
                # print("text_of_perturbed_imgs:")
                # print(text_of_perturbed_imgs)
                # text_of_perturbed_imgs = [i[:77] for i in text_of_perturbed_imgs]
            ...
            coefficient = torch.sum((perturb_text_features - adv_text_features) * tgt_text_features,
                                    dim=-1)  # size = (num_query * batch_size)
            ...
            clamp_before = image_clean + delta
            ...
            with torch.no_grad():
                ...
                # print(lavis_text_of_adv_image_in_current_step)
                # lavis_text_of_adv_image_in_current_step = [i[:77] for i in lavis_text_of_adv_image_in_current_step]
                lavis_text_token = clip.tokenize(lavis_text_of_adv_image_in_current_step,truncate=True).to(device)
                lavis_text_features_of_adv_image_in_current_step = clip_img_model_vitb32.encode_text(lavis_text_token)
                lavis_text_features_of_adv_image_in_current_step = lavis_text_features_of_adv_image_in_current_step / lavis_text_features_of_adv_image_in_current_step.norm(
                    dim=1, keepdim=True)
                lavis_text_features_of_adv_image_in_current_step = lavis_text_features_of_adv_image_in_current_step.detach()

                adv_txt_tgt_txt_score_in_current_step = torch.mean(
                    torch.sum(lavis_text_features_of_adv_image_in_current_step * tgt_text_features, dim=1)).item()
                ...
                if adv_txt_tgt_txt_score_in_current_step > query_attack_results[i]:
                    ...
                    img_path=os.path.dirname(os.path.dirname(args.output))
                    filename = '{:06d}.png'.format(len(log))
                    folder_to_save = os.path.join(img_path, 'best_adv_image')
                    if not os.path.exists(folder_to_save):
                        os.makedirs(folder_to_save, exist_ok=True)
                    torchvision.utils.save_image(clamp_before, os.path.join(folder_to_save, filename))
                    better_flag = 1
                ...
                text_features_of_adv_image_in_current_step_rn50 = text_features_of_adv_image_in_current_step_rn50 / text_features_of_adv_image_in_current_step_rn50.norm(
                    dim=1, keepdim=True)
                text_features_of_adv_image_in_current_step_rn50 = text_features_of_adv_image_in_current_step_rn50.detach()
                adv_txt_tgt_txt_score_in_current_step_rn50 = torch.mean(
                    torch.sum(text_features_of_adv_image_in_current_step_rn50 * tgt_text_features_rn50, dim=1)).item()
                ...
                text_features_of_adv_image_in_current_step_rn101 = text_features_of_adv_image_in_current_step_rn101 / text_features_of_adv_image_in_current_step_rn101.norm(
                    dim=1, keepdim=True)
                text_features_of_adv_image_in_current_step_rn101 = text_features_of_adv_image_in_current_step_rn101.detach()
                adv_txt_tgt_txt_score_in_current_step_rn101 = torch.mean(
                    torch.sum(text_features_of_adv_image_in_current_step_rn101 * tgt_text_features_rn101, dim=1)).item()
                ...
                text_features_of_adv_image_in_current_step_vitb16 = text_features_of_adv_image_in_current_step_vitb16 / text_features_of_adv_image_in_current_step_vitb16.norm(
                    dim=1, keepdim=True)
                text_features_of_adv_image_in_current_step_vitb16 = text_features_of_adv_image_in_current_step_vitb16.detach()
                adv_txt_tgt_txt_score_in_current_step_vitb16 = torch.mean(
                    torch.sum(text_features_of_adv_image_in_current_step_vitb16 * tgt_text_features_vitb16,
                              dim=1)).item()
                ...
                text_features_of_adv_image_in_current_step_vitl14 = text_features_of_adv_image_in_current_step_vitl14 / text_features_of_adv_image_in_current_step_vitl14.norm(
                    dim=1, keepdim=True)
                text_features_of_adv_image_in_current_step_vitl14 = text_features_of_adv_image_in_current_step_vitl14.detach()
                adv_txt_tgt_txt_score_in_current_step_vitl14 = torch.mean(
                    torch.sum(text_features_of_adv_image_in_current_step_vitl14 * tgt_text_features_vitl14,
                              dim=1)).item()
        ...
        log.append(
    {
        "moving-avg-adv-rn50": float(np.mean(vit_attack_results_rn50[:(i + 1)])),
        "moving-avg-query-rn50": float(np.mean(query_attack_results_rn50[:(i + 1)])),

        "moving-avg-adv-rn101": float(np.mean(vit_attack_results_rn101[:(i + 1)])),
        "moving-avg-query-rn101": float(np.mean(query_attack_results_rn101[:(i + 1)])),

        "moving-avg-adv-vitb16": float(np.mean(vit_attack_results_vitb16[:(i + 1)])),
        "moving-avg-query-vitb16": float(np.mean(query_attack_results_vitb16[:(i + 1)])),

        "moving-avg-adv-vitb32": float(np.mean(vit_attack_results[:(i + 1)])),
        "moving-avg-query-vitb32": float(np.mean(query_attack_results[:(i + 1)])),

        "moving-avg-adv-vitl14": float(np.mean(vit_attack_results_vitl14[:(i + 1)])),
        "moving-avg-query-vitl14": float(np.mean(query_attack_results_vitl14[:(i + 1)])),
    }
)
        with open(os.path.join(args.output + '.json'), 'w') as f:
        ...
        with open(os.path.join(args.output + '.txt'), 'a') as f:
            ...
            if better_flag:
                ...
                f.write(best_caption)
                img_path = os.path.dirname(os.path.dirname(args.output))
                filename = '{:06d}.png'.format(len(log)-1)
                folder_to_save = os.path.join(img_path, 'best_adv_image')
                if not os.path.exists(folder_to_save):
                    os.makedirs(folder_to_save, exist_ok=True)
                torchvision.utils.save_image(clamp_before, os.path.join(folder_to_save, filename))
